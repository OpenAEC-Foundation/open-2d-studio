...
import type {
  Shape, TextAlignment, TextVerticalAlignment,
  ...
  SlabShape, SlabMaterial, SlabLabelShape,
  SectionCalloutShape, SpaceShape,
  PlateSystemShape,
...
export function ParametricShapeProperties({ shape }: { shape: ParametricShape }) {
...
export function MultiSelectShapeProperties({
  shapes,
  updateShape
}: {
  shapes: Shape[];
...
export function GridlineSpacingInput({ gridline }: { gridline: GridlineShape }) {
...
export function BeamProfileSearch({ beam, update }: { beam: BeamShape; update: (updates: Record<string, unknown>) => void }) {
...
export function ShapeProperties({ shape, updateShape }: { shape: Shape; updateShape: (id: string, updates: Partial<Shape>) => void }) {
...
export function PileSymbolPicker({
  contourType,
  fillPattern,
  onChange,
}: {
...
export function PileShapeProperties({ pile, update }: { pile: PileShape; update: (updates: Record<string, unknown>) => void }) {
...
export function PlateSystemShapeProperties({ shape, updateShape }: { shape: PlateSystemShape; updateShape: (id: string, updates: Partial<Shape>) => void }) {
  const update = (updates: Record<string, unknown>) => updateShape(shape.id, updates as Partial<Shape>);
  const unitSettings = useUnitSettings();
  const ps = shape;
